...
const MedicalSuppliesCRUD = ({ setLastSavedRecord }) => {
  ...
  const [supplyForm, setSupplyForm] = useState(initialFormState);

  const resetForm = () => {
    setSupplyForm(initialFormState);
  };
    ...
    const fetchInventory = async () => {
      ...
      try {
        ...
        setErrorSnackBar({
          open: true,
          message:
            err.response?.data?.message || "Không thể tải danh sách vật tư y tế",
        });
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const validateForm = (isEdit = false) => {
    if (!supplyForm.itemName.trim()) return "Tên vật tư y tế là bắt buộc";
    if (!supplyForm.unit) return "Đơn vị là bắt buộc";
    if (!isEdit) {
      if (!supplyForm.quantity || parseInt(supplyForm.quantity) < 0)
        return "Số lượng phải là số không âm";
      if (!supplyForm.expirationDate) return "Ngày hết hạn là bắt buộc";
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const tomorrow = new Date(today);
      tomorrow.setDate(today.getDate() + 1);
      const selectedDate = new Date(supplyForm.expirationDate).setHours(
        0,
        0,
        0,
        0
    ...
    if (
      !supplyForm.lowStockThreshold ||
      parseInt(supplyForm.lowStockThreshold) < 0
    )
      return "Ngưỡng tồn kho thấp phải là số không âm";
    if (isEdit && !supplyForm.status) return "Trạng thái là bắt buộc";
    return null;
  };

  const handleSubmitNewSupply = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      const payload = {
        ...
        type: "MEDICAL_SUPPLIES",
        lowStockThreshold: parseInt(supplyForm.lowStockThreshold),
        quantity: parseInt(supplyForm.quantity),
        expirationDate: supplyForm.expirationDate + "T00:00:00.000Z",
      };
      const response = await inventoryService.stockIn(payload);
      setSupplyRecords((prev) => [...prev, response.data]);
      ...
      const newTotalPages = Math.ceil(
        (supplyRecords.length + 1) / itemsPerPage
      );
      ...
      setErrorSnackBar({
        open: true,
        message: err.response?.data?.message || "Không thể thêm vật tư y tế mới",
      });
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const handleEditSupply = async (e) => {
    ...
    try {
      ...
      setErrorSnackBar({
        open: true,
        message: err.response?.data?.message || "Không thể cập nhật vật tư y tế",
      });
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const handleEditBatch = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        newQuantity: parseInt(supplyForm.quantity),
        reason: `Cập nhật lô, ngày hết hạn: ${new Date(
          supplyForm.expirationDate
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setErrorSnackBar({
        open: true,
        message:
          err.response?.data?.message || "Không thể ngừng sử dụng vật tư y tế",
      });
      console.error(err);
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
            ...
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-lg text-gray-600 font-medium leading-relaxed">
                  Quản lý tất cả vật tư y tế có trong kho.
                </p>
              </div>
            </div>
          </div>
      ...
      >
        <TextField
          placeholder="Tìm kiếm theo tên vật tư y tế"
          value={searchTerm}
          onChange={(e) => {
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>Thêm Vật Tư Y Tế Mới vào Kho</DialogTitle>
        <DialogContent>
          <Stack spacing={3} sx={{ mt: 1 }}>
            ...
            <TextField
              ...
              label="Ngày hết hạn"
              type="date"
              value={supplyForm.expirationDate}
              onChange={handleFormChange}
              required
              fullWidth
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>Chỉnh sửa Vật Tư Y Tế</DialogTitle>
        <DialogContent>
          <Stack spacing={3} sx={{ mt: 1 }}>
            <TextField
              name="itemName"
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Vật tư y tế <strong>{lastSavedRecord?.itemName}</strong> đã được thêm
            vào kho.
          </Typography>
        </DialogContent>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Bạn có chắc chắn muốn ngừng sử dụng vật tư y tế này không?
          </Typography>
        </DialogContent>
...
export default MedicalSuppliesCRUD;
