...
import { defaultLogs } from "~/mock/mock";
import { AccessTime, EventNote, LocalPharmacy, Medication, Person, Schedule, Warning } from "@mui/icons-material";

const MedicineLogPage = ({ dispensedLogs }) => {
  const logs = dispensedLogs || defaultLogs;
  ...
  const itemsPerPage = 9; // Adjusted to 9 to align with 3 per row
    ...
    return logs.filter((log) => {
      const matchesSearch =
        ...
        log.event.studentId
          .toLowerCase()
          .includes(logSearchTerm.toLowerCase()) ||
        log.medications.some((med) =>
          med.name.toLowerCase().includes(logSearchTerm.toLowerCase())
        );
      const matchesDate =
        logDateFilter === "" || log.event.date === logDateFilter;
      const matchesClass =
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: "flex", gap: 2, mb: 3, flexWrap: "wrap" }}>
        <TextField
          label="Tìm kiếm theo tên thuốc, học sinh, sự kiện..."
          value={logSearchTerm}
      ...
      <Grid container spacing={2} sx={{ maxHeight: "60vh", overflowY: "auto" }}>
        {paginatedLogs.length > 0 ? (
          paginatedLogs.map((log) => (
            <Grid item xs={4} key={log.event.id}>
              <Card
                sx={{
                  boxShadow: 2,
                  borderRadius: 2,
                ...
                <CardContent sx={{ flexGrow: 1, overflow: "hidden" }}>
                  ...
                  <Box sx={{ mb: 1, overflow: "hidden" }}>
                    ...
                    >
                      ...
                      {log.event.studentName} ({log.event.studentId})
                    </Typography>
                    <Typography
                      variant="body1"
                      sx={{
                    ...
                    >
                      <strong>Mô tả:</strong> {log.event.description}
                    </Typography>
                    <Typography
                      variant="caption"
                      color="text.secondary"
                    ...
                    >
                      <Clock size={14} sx={{ mr: 1 }} /> {log.event.date}{" "}
                      {log.event.time}
                    </Typography>
                  </Box>
                  <Divider sx={{ my: 1 }} />
          ...
          <Box sx={{ p: 3 }}>
            ...
            >
              ...
              >
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                  ...
                  >
                    <School sx={{ color: "#16a34a", fontSize: 20 }} />
                  </Box>
                  <Box>
                    <Typography
                      variant="body2"
                ...
                >
                  ...
                  <Box>
                    ...
                    <Typography variant="body1" fontWeight="600">
                      {selectedLog?.event.date} - {selectedLog?.event.time}
                    </Typography>
                  </Box>
                </Box>
              </Box>
            ...
            >
              ...
              >
                ...
                Thuốc đã cấp phát ({selectedLog?.medications.length} loại)
              </Typography>

              <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                {selectedLog?.medications.map((med, index) => (
                  <Box
                    key={med.id}
                    sx={{
                      bgcolor: "white",
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        />
                        <strong>Cách sử dụng:</strong> {med.usage}
                      </Typography>
                      <Typography
                        variant="caption"
                        color="text.secondary"
